...
describe("NostrRelayPool", () => {
  ...
  describe("connection management", () => {
    ...
    it("keeps increasing backoff when relay flaps quickly after opening", () => {
      vi.useFakeTimers();
      try {
        const pool = new NostrRelayPool({ reconnectInterval: 100 });
        pool.connect("wss://relay.example.com");
    ...
    it("resets backoff to base interval after a stable connection window", () => {
      vi.useFakeTimers();
      try {
        const pool = new NostrRelayPool({ reconnectInterval: 100 });
        pool.connect("wss://relay.example.com");
