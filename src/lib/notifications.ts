...
export function notifyNeedSigninModify(): void {
  toast.error(i18n.t("toasts.errors.needSigninModify"));
}

export function notifyNeedSigninPost(): void {
  toast.error(i18n.t("toasts.errors.needSigninPost"));
}

export function notifyStatusRestricted(): void {
  toast.error(i18n.t("toasts.errors.statusRestricted"));
}

export function notifyDisconnectedSelectedFeeds(): void {
  toast.warning(i18n.t("toasts.warnings.disconnectedSelectedFeeds"), { id: "disconnected-selected-feeds" });
}

// Relay filter

export function notifyRelayFilterDisabled(relayDomain: string): void {
  toast(i18n.t("toasts.success.relayFilterDisabled", { relayDomain }));
}

export function notifyRelayFilterEnabled(relayDomain: string): void {
  toast(i18n.t("toasts.success.relayFilterEnabled", { relayDomain }));
}

export function notifyShowingOnlyRelay(relayDomain: string): void {
  toast(i18n.t("toasts.success.showingOnlyRelay", { relayDomain }));
}

export function notifyRelayFiltersCleared(): void {
  toast(i18n.t("toasts.success.relayFiltersCleared"));
}

export function notifyAllRelaysSelected(): void {
  toast(i18n.t("toasts.success.allRelaysSelected"));
}

// Relay reconnect

export function notifyRelayReconnectFailed(relayDomain: string): void {
  toast.error(i18n.t("toasts.errors.relayReconnectFailedDeselected", { relayDomain }));
}

export function notifyRelayReconnectAttempt(relayDomain: string): void {
  toast.info(i18n.t("toasts.info.relayReconnectAttempt", { relayDomain }));
}

// Channel / person filter

export function notifyShowingOnlyChannel(channelName: string): void {
  toast(i18n.t("toasts.success.showingOnlyChannel", { channelName }));
}

export function notifyAllChannelsReset(): void {
  toast(i18n.t("toasts.success.allChannelsReset"));
}

export function notifyShowingOnlyTag(tag: string): void {
  toast(i18n.t("toasts.success.showingOnlyTag", { tag }));
}

export function notifyNoFrequentPeople(): void {
  toast(i18n.t("toasts.success.noFrequentPeople"));
}

export function notifyFrequentPeopleDeselected(): void {
  toast(i18n.t("toasts.success.frequentPeopleDeselected"));
}

function resolvePersonToastName(person?: Person | null): string {
  if (!person) return i18n.t("toasts.success.selectedUserFallback");
  ...
  return i18n.t("toasts.success.selectedUserFallback");
}

export function notifyShowingOnlyPersonExclusive(person?: Person | null): void {
  toast(i18n.t("toasts.success.showingOnlyPersonExclusive", { personName: resolvePersonToastName(person) }));
}
...
export function notifyPublishStatusFailed(): void {
  toast.error(i18n.t("toasts.errors.publishStatusFailed"));
}

export function notifyPublishDateFailed(): void {
  toast.error(i18n.t("toasts.errors.publishDateFailed"));
}

export function notifyPublishPriorityFailed(): void {
  toast.error(i18n.t("toasts.errors.publishPriorityFailed"));
}

// Publish: listing

export function notifyPublishListingStatusFailed(): void {
  toast.error(i18n.t("toasts.errors.publishListingStatusFailed"));
}

// Publish: new task flow

export function notifyNeedTag(): void {
  toast.error(i18n.t("toasts.errors.needTag"));
}

export function notifyTaskCreationFailed(): void {
  toast.error(i18n.t("toasts.errors.taskCreationFailed"));
}
...
export function notifyPendingPublish(durationMs: number, onUndo: () => void): string | number {
  return toast(i18n.t("toasts.info.pendingPublish", { seconds: Math.floor(durationMs / 1000) }), {
    duration: durationMs,
    action: {
      label: i18n.t("toasts.actions.undo"),
      onClick: onUndo,
    },
  });
}
...
export function notifyPublishUndone(): void {
  toast.info(i18n.t("toasts.success.publishUndone"));
}

export function notifyRetryRelayMissing(): void {
  toast.error(i18n.t("toasts.errors.retryRelayMissing"));
}

export function notifyRetryRejectedByRelay(reason?: string): void {
  if (reason) {
    toast.error(i18n.t("toasts.errors.retryRejectedByRelayWithReason", { reason }));
  } else {
    toast.error(i18n.t("toasts.errors.retryRejectedByRelay"));
  }
}
...
export function notifyPublished(
  ...
  if (resolvedSpaceNames.length > 0) {
    toast.success(i18n.t("toasts.success.publishedToSpaces", { spaceNames: resolvedSpaceNames.join(", ") }));
    return;
  }
  toast.success(taskType === "comment" ? i18n.t("toasts.success.publishedComment") : i18n.t("toasts.success.publishedTask"));
}

export function notifyLocalSaved(taskType: TaskEntryType): void {
  toast.success(taskType === "comment" ? i18n.t("toasts.success.localComment") : i18n.t("toasts.success.localTask"));
}
...
export function notifyPartialPublish(options: { publishedCount: number; targetCount: number }): void {
  toast.warning(i18n.t("toasts.warnings.partialPublish", options));
}

export function notifyPublishSavedForRetry(options: PublishRetryToastOptions = {}): void {
  const { relayUrl, reason } = options;
  if (relayUrl && reason) {
    toast.error(i18n.t("toasts.errors.publishSavedForRetryWithRelayReason", { relayUrl, reason }));
    return;
  }
  if (reason) {
    toast.error(i18n.t("toasts.errors.publishSavedForRetryWithReason", { reason }));
    return;
  }
  toast.error(i18n.t("toasts.errors.publishSavedForRetry"));
}
