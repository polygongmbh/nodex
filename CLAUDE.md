...
When the user says `squash` or `release` (or `push` in the release sense), invoke the matching skill under `.claude/skills/` — these encapsulate the full procedures (history rewrite rules for `squash`; reconciliation, version bump, verification, tag, and push for `release`). Never shortcut directly to `git push` for a release unless the user explicitly bypasses the routine.
