...
The UI supports multiple task views (status, feed, kanban, and calendar by default; tree and list reachable via direct URL), nested subtasks, comments, NIP-52 calendar events, channel-based filtering, and mobile-first compose/search controls.
...
- `VITE_VIEWS`: optional comma-separated list of views surfaced in navigation and auto-redirects
  (`home`, `status`, `feed`, `tree`, `kanban`, `list`, `calendar`); blank shows all. When only one
  view is listed, the navigation bar is hidden on desktop and mobile. Views left out stay reachable
  by typing their URL directly.
