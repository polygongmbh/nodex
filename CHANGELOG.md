...
- Breadcrumb labels no longer pre-abbreviate task text, now stay left-aligned on a single line, distribute space evenly across multi-level page and task-card paths, and only truncate when layout space is actually constrained.
