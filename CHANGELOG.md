...
- The app no longer persists raw Nostr events to localStorage. Tasks, comments, listings, calendar events, reactions and status updates are projected at ingest time into typed in-memory stores (Posts, reactions registry) and read from there; the wire-level event bag now lives only inside the ingestion hook for the duration of the session. Cold starts begin with an empty timeline that fills as the subscribed relays return events. Frees several hundred KB of localStorage on busy scopes and removes a long-standing source of memory growth.
