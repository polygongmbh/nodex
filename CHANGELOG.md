...
- The app no longer persists raw Nostr events to localStorage. Tasks, comments, listings, calendar events, reactions and status updates are projected at ingest time into typed in-memory stores (Posts, reactions registry) and read from there; the wire-level event bag now lives only inside the ingestion hook for the duration of the session. A new per-relay-scope Post cache keeps the timeline visible across reloads — it persists already-converted Posts (not raw events) and gets refreshed once each session's live subscription has caught up. Frees several hundred KB of localStorage on busy scopes and removes a long-standing source of memory growth.
- The per-relay Post cache now keeps up to 2000 posts per relay (was 500), so reloads on busy relays no longer drop history the user just had on screen.
