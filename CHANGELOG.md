...
- Opening a permalink shaped `DOMAIN/RELAY[/TASKID]` now preserves the existing relay selection when that relay is already part of the persisted list. Previously the persisted list was always replaced with just the path-derived relay; now we only narrow when the relay isn't currently active.
