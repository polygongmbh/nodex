...
- Calendar day cards for multi-day timed events now reflect what's actually happening that day: the first day shows `from HH:mm`, intermediate days show `All day`, and the last day shows `until HH:mm`, instead of repeating the same `HH:mm – HH:mm` range on every day in the span.
