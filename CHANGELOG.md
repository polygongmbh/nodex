...
- Breadcrumb labels no longer pre-abbreviate task text, now stay left-aligned on a single line, distribute space evenly across multi-level paths, and only truncate when layout space is actually constrained.
