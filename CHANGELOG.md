...
## [Unreleased]
- Auth dialogs now use consistently rounded corners across the app, and the sign-in chooser now has a compact width-constrained layout with Noas full-width first, Signer/Extension and Guest/Private row ordering, refreshed signer icon treatment, guest-entry env gating (`VITE_ALLOW_GUEST_SIGN_IN`), and a persistent horizontal `username @ host` Noas field layout with a simplified host placeholder (`example.com`).
