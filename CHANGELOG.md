...
- The desktop sidebar gained a Projects section listing active top-level tasks with open subtasks, plus indented active subprojects (subtasks that themselves have an active subtask); clicking a row focuses that project and the header's folder icon jumps back to the root. In every view the rows containing the focused post are highlighted and temporary indented entries trace the path down to the focused post, replacing the breadcrumb bar in Home.
