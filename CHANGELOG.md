...
## [4.0.0] - 2026-05-28
Major release for NIP-52 calendar event composing, a typed in-memory ingest pipeline replacing raw-event localStorage, slimmer top-nav, and broad composer/feed/calendar polish.
...
- The read-side converter no longer scrapes every body URL into `task.attachments`. Previously a plain hyperlink in the body was indexed as an attachment and rendered as a URL link chip under the post, even when no imeta tag was published for it. The converter now trusts imeta / nip-94 tags only. Body URLs with media file extensions that the author *did* tag with imeta (e.g. images pasted into the body — Nodex auto-imetas these on publish) still surface as attachments via the imeta path; bare hyperlinks no longer become phantom "attachments".
- Dropped `aria-label` across the app in favor of the existing `title` hover tooltip. Nodex is a visual-first app and the duplication was pure noise — most icon-only buttons carried the same i18n string in both attributes. Where they diverged (breadcrumb back/up, publish-queue retry/repost, composer signin/submit, task status menus, channel/relay toggles), the richer/conditional `title` text wins. Orphaned aria-only i18n keys have been removed from all three locales.
...
- Hover/`title` tooltips on feed, calendar, tree, and upcoming cards now reflect the actual post type — calendar events read "Focus event", NIP-99 listings read "Focus listing", and comments read "Focus comment" instead of all collapsing to "Focus task".
...
- Channel ranking now counts posts from the converted timeline instead of every raw Nostr event. Status updates, reactions, deletions, and other auxiliary events no longer inflate a channel's usage count, so a channel with many state transitions on a single task is no longer treated as more active than one with several real posts. The involvement signal also scales with the number of posts you've authored in a channel instead of being a flat one-or-zero flag, so heavily user-posted channels rank visibly above busy channels you've never contributed to.
- The app no longer persists raw Nostr events to localStorage. Tasks, comments, listings, calendar events, reactions and status updates are projected at ingest time into typed in-memory stores (Posts, reactions registry) and read from there; the wire-level event bag now lives only inside the ingestion hook for the duration of the session. A single Post cache keeps the timeline visible across reloads — it persists already-converted Posts (not raw events), is loaded once at startup, and is flushed on tab-hide / unload after the live subscription has caught up. Frees several hundred KB of localStorage on busy scopes and removes a long-standing source of memory growth.
- The Post cache now keeps up to 5000 posts (was 500 per relay), so reloads on busy relays no longer drop history the user just had on screen.
- The feed loading indicator now clears as soon as relays go quiet (~1.5s with no new events) instead of waiting for every subscribed relay to emit EOSE. One slow relay no longer holds the spinner over an already-rendered feed.
- Profile (kind 0) ingest now batches by relay bucket during backfill instead of merging the whole bucket once per incoming event. A backfill burst of N profiles across M relays used to allocate roughly 10 large arrays per event (an `existing+1` spread, a flat/sort/slice merge, and a cross-relay re-read) — it now collapses to one merge per relay per debounce window. Cuts steady-state array count by an order of magnitude on profile-heavy scopes and reduces post-EOSE GC pressure.
...
### Fixed
- Calendar events no longer silently drop a typed end-time when the start has no time (or vice versa). The all-day decision now requires *both* sides to be timeless; any time on either side promotes the event to a timed (kind 31923) publish, with the missing side defaulting to 00:00. The composer also surfaces a warning toast so the inferred time is visible.
- Calendar cards no longer show the raw URL as a caption for an attached image or video that has no `alt`/`name`. The calendar card's media-caption derivation was diverging from the feed's (it fell back to the URL string instead of leaving the caption empty); both surfaces now share a single helper and skip captionless attachments.
- Locally posted tasks (and other posts) now appear in the feed immediately again. NDK's own optimistic publish-echo carries no relay attribution, so the typed ingest boundary was dropping it as "no relay" and the post stayed invisible until/unless the upstream relay independently echoed it back. The publish layer now feeds the just-signed event into the typed ingest pipeline itself, using the relays that actually acked, while NDK's relay-less optimistic dispatch is suppressed at the subscription level.
- Deleting (or recomposing) a calendar event or NIP-99 listing now actually removes it. These kinds are parameterized-replaceable, so the kind-5 deletion event now carries the NIP-01 address tag (`a` = `<kind>:<pubkey>:<d>`) alongside the event id — relays that index these kinds by address (not id) now honor the deletion and stop re-serving the old version. Inbound `a`-tag deletions from other clients are also processed, and the store's address → id map is now released when its post is deleted so a fresh re-publish at the same address isn't blocked.
- Updating a task's due date (or any other date) now actually re-renders. The composer was writing the change optimistically to a parallel `localTasks` snapshot that the merge tiebreaker preferred over the live posts-store entry on id collision, so the calendar event the relay echoed back never made it onto the visible task. Date changes are now solely round-trip-driven; the optimistic shadow is gone, and the placeholder still used during the publish-undo window self-cleans once the live event lands.
- Recomposing or deleting a post no longer briefly resurrects the old version. The localStorage Post cache, used to hydrate the timeline before live events arrive, is now dropped from the merge as soon as the live subscription finishes its initial replay — previously the cached copy kept overriding the store's deletion/suppression, so a freshly deleted or recomposed post would stay on screen until the next reload.
- The composer no longer wipes out your draft when a publish is rejected. The post-submit clear now waits for the publish flow's result — on success the composer clears as before, on failure (e.g. a task submission rejected because multiple relays are active) the content, tags, mentions, attachments, dates, location, listing/event metadata, and the recompose banner all stay intact. The submit button is disabled while a publish is in flight to prevent double-submission.
- Calendar events in the feed (and tree/list/kanban/status views) no longer get greyed out when their start date is in the future, and their date chip is no longer tinted red/yellow/green by proximity. Urgency colouring now applies only to `due` and `scheduled` dates — the date types that represent work the user owes — while events show in a neutral colour.
- Calendar view no longer crashes with `Can't find variable: setIsComposingEvent` when navigating across days; the day-change effect now resets the new composerMode state introduced by the two-button add affordance.
- Clicking "Create Event" on the calendar's selected-day panel now actually creates a calendar event instead of silently producing a task. The composer's allowComment guard used to coerce any non-task mode (Event, Listing) back to Task whenever comments were disabled by the caller — it now only overrides Comment, leaving the explicit Event/Listing choice intact.
- The feed chip for a multi-day event now shows just the date range (e.g. `May 16, 2026 – May 19`) instead of pinning a misleading single time-of-day to the whole span. Same-day events keep their start time and end-time hint.
- Calendar day cards for multi-day timed events now show a meaningful per-day range instead of repeating the same `HH:mm – HH:mm` on every day in the span: the first day reads `HH:mm – <end-date>`, intermediate days read `<start-date> – <end-date>`, and the last day reads `<start-date> – HH:mm`.
- Clicking "Add Event" on the calendar's selected-day panel now actually opens the composer in Event mode. A persisted draft (e.g. a half-written feed post) no longer overrides the explicit defaultPostType passed in by the calling button, and the composer remounts when switching between Add Task and Add Event so the new mode takes effect.
- Adding or removing a single relay no longer opens a fresh WebSocket for every other relay in the list. The app was re-assigning NDK's `explicitRelayUrls` property on every relay change, which triggered NDK's pool monitor to re-drive `connect()` against the entire list; the array is now mutated in place so only the affected relay's socket is opened or closed.
- Re-adding a relay that was just removed now restores its feed. Previously the in-memory event cache was stripped of the removed relay's URL across every scope bucket on remove, and the freshly-rejoined relay was never flagged for subscription replay — so the global subscription's REQ was never sent to the new socket. The cache strip is gone (the inactive scope isn't rendered anyway) and `addRelay` now marks the relay for replay so events flow back as soon as it connects.
- Stopped NDK from silently connecting to relays that aren't in the configured/connected list. Two NDK defaults were responsible: `enableOutboxModel` opened a hidden "outbox pool" pinned to `wss://purplepag.es` and `wss://nos.lol` on every page load (signed in or not), and `autoConnectUserRelays` added every relay advertised in the signed-in user's kind 3 / kind 10002 events. Both are now disabled, so the only relays the app talks to are the ones the sidebar shows.
